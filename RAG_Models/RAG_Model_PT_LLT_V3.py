"""
File Name: RAG_Model_PT_LLT_V3.py    === Author: Naghme Dashti / August 2025
...
OUTPUT_FILE_NAME         = "Delta_output_v3_PT"       # output file name
...
for idx, row in ae_df.iterrows():
    ...
    try:
        ...
        # Old term metrics 
        exact_LLT_match = (true_term is not None and answer_line == true_term)
        LLT_fuzzy_score = fuzz.ratio(answer_line.lower(), (true_term or "").lower())
        LLT_fuzzy_match = (true_term is not None and LLT_fuzzy_score >= 90)
        ...
        print(f"→ LLT_correct: {llt_correct} | PT_correct: {pt_correct} | Exact LLT: {exact_LLT_match}, Fuzzy LLT: {LLT_fuzzy_score} | Exact PT: {exact_PT_match}, Fuzzy PT: {PT_fuzzy_score:.1f}\n")
        time.sleep(0.1)
...
print(f"\n PT Term Accuracy (exact): {pt_acc:.2f}")
