...
def list_server_jsons():
    """List *.json files from RAG_Models"""
...
def _write_full_snapshot(data, results, out_path_full, reviewer, default_unreviewed=False):
    """
    Write a full file that has exactly as many entries as input.
    - Reviewed: use results (True/False).
    - Not reviewed: manual check = default_unreviewed (False).
    """
    decided = {
...
# ====== Core fns ======
def load_data(reviewer_display, selected_path, use_auto_accept, request: gr.Request):
    """
    - Load selected file
    - auto-accept exact matches
    - write/update checked & full outputs
    - return the first item needing manual check
    """
    # force reviewer from auth username
    reviewer = (getattr(request, "username", None) or "").strip()

    if not reviewer:
        return (gr.update(value=""), "Auth username missing.",
                None, None, None, None, None, reviewer, gr.update(visible=False))
    ...
    if use_auto_accept:
        ...
        for e in data:
            ...
            if e.get("exact_LLT_match", False) is True:
                e2 = dict(e)
                e2["manual check"] = True
                e2["reviewer"] = reviewer
                e2["reviewed_at"] = utc_iso()
                results.append(e2)
                done.add(k)
                changed = True
        if changed:
    ...
    # always write FULL (unchecked = False)
    ...
    if idx is None:
        status = (f" All entries reviewed for: {os.path.basename(path)}\n"
                  ...
                  f"Reviewer: {reviewer}")
        # also update the readonly reviewer box
...
def save_and_next(choice, state_data, state_results, out_checked, out_full, idx, reviewer_state, request: gr.Request):
    # force reviewer from auth username
    reviewer = (getattr(request, "username", None) or reviewer_state or "").strip()
...
def autofill_reviewer(request: gr.Request):
    """Put auth username into the readonly reviewer box."""
    return getattr(request, "username", "") or ""
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        reviewer_box = gr.Textbox(label="Reviewer (from login)", value="", interactive=False, scale=2)
    ...
    # auto-fill reviewer box from auth on page load
    demo.load(autofill_reviewer, inputs=None, outputs=reviewer_box)

    load_btn.click(
        load_data,
        inputs=[reviewer_box, dataset, use_auto],
...
if __name__ == "__main__":
    demo.launch(
        share=True,
        auth=[("reviewer1","987asd"), ("reviewer2","123qwe"), ("reviewer3","123asd")]
    )
