# visualize_llt_embeddings_by_SOC.py
...
# Limit to 5000 samples randomly for UMAP visualization
if len(full_df) > 5000:
...
umap_model = umap.UMAP(n_neighbors=200, min_dist=0.99, metric='cosine', random_state=42)
...
num_socs = len(unique_socs)

# Generate different RGB Colors
...
# Create color Palette for SOCs
distinct_colors = generate_distinct_colors(num_socs)
soc_to_color = {soc: distinct_colors[i] for i, soc in enumerate(unique_socs)}
colors = [soc_to_color[soc] for soc in full_df['SOC_Term']]
...
plt.tight_layout()
plt.grid(False)
plt.savefig("./Visualization/5000_llt_embeddings_by_SOC1.png", dpi=300)
plt.show()
