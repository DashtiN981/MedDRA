...
plt.tight_layout()
plt.savefig("./Visualization/UMAP_Top10_SOC_LLTS_HullOnAll.png", dpi=300)
...
# output
fig.write_html("./Visualization/UMAP_Top10_SOC_LLTS_HullOnAll_Interactive.html")
fig.show()

# Filter for specific SOC
...
for _, row in target_df.iterrows():
    plt.text(row["x"] + 0.2, row["y"], row["LLT_Term"], fontsize=7, alpha=0.7)

plt.title(f"Zoomed UMAP for {target_soc_term}", fontsize=14)
plt.axis("off")
plt.tight_layout()
plt.savefig("./Visualization/Zoom_LLTs_BloodLymph.png", dpi=300)
plt.show()
