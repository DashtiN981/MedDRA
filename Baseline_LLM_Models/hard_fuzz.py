...
try:
    ...
    # 1) If any row has Ist_Primary_SOC == 'Y' -> that row's SOC_Code is primary
    # 2) Else if Primary_SOC_Code column exists and is uniquely defined (one unique non-null) -> use that
    # 3) Else if there is exactly one SOC_Code for the PT -> use that
    # 4) Else -> None (undefined)
    ...
    num_undefined = sum(1 for k, v in pt_code_to_primary_soc.items() if v is None)
