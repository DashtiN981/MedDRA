# === File Name: Baseline_zero_shot_LLTMeddra.py ===
# Author: Naghme Dashti / (Baseline 0 – Zero-shot MedDRA LLT prediction)
# Fixed by: ChatGPT (null-safe output, retry, no-skip, NameError-proof, correct meta model)
# Description:
#   - Zero-shot baseline: LLM sees ONLY the AE description (no candidate list).
#   - We ask it to output ONE MedDRA LLT term.
#   - Afterwards, we check whether the predicted term is:
#       (a) a valid MedDRA LLT from our dictionary
#       (b) equal to the human baseline LLT (exact / fuzzy)
#   - PT/SOC metrics are computed like in fuzz baseline (code-based).
#
# Key fixes:
#   1) message.content can be None -> null-safe
#   2) retry on empty/None outputs
#   3) no NameError in except (all vars initialized)
#   4) meta model name matches actual model
#   5) keeps "no-skip" behavior: errors become rows
...
OUT_JSON     = "/home/naghmedashti/MedDRA-LLM/Baseline_LLM_Models/Mosaic_output_zeroshot1.json"
...
# ---------------------------
# Canonicalize numeric-like codes
# ---------------------------
def canon_code(x):
    if x is None:
        return None
...
# ---------------------------
# OpenAI-compatible client (adjust API key / base_url)
# ---------------------------
client = OpenAI(
...
# ---------------------------
# Helper: robust LLM call (retry + non-empty)
# ---------------------------
def call_llm_zero_shot(prompt: str, model: str, retries: int = 3, sleep_sec: float = 1.0) -> str:
    last_err = None
    for attempt in range(retries):
        try:
            ...
            if text:
                return text
            last_err = ValueError("Empty/None model output (message.content is None/empty).")
        except Exception as e:
            last_err = e
        time.sleep(sleep_sec)

    raise RuntimeError(f"LLM failed after {retries} retries. Last error: {last_err}")
...
for _, r in llt_df.iterrows():
    key = str(r["LLT_Term"]).strip().lower()
    if key and key not in llt_term_to_code:
        llt_term_to_code[key] = r["LLT_Code"]



# For zero-shot: allow fuzzy mapping of predicted term to a valid LLT code (Newhardrapidfuzz-style)
all_llt_terms = llt_df["LLT_Term"].tolist()

def map_pred_term_to_llt_code(pred_term: str):
    """Map predicted LLT term -> LLT_Code.
    Priority:
      1) exact match against global LLT dictionary
      2) fuzzy match against *all* LLT terms (since zero-shot has no candidate list)
    Returns canonical LLT_Code or None.
    """
    ...
    # fuzzy to the full LLT dictionary
    best = process.extractOne(key, (s.lower() for s in all_llt_terms), scorer=fuzz.ratio, score_cutoff=70)
    if best:
        best_term_lower = best[0]
        return llt_term_to_code.get(best_term_lower)
...
# Optional PT_Term from LLT file
...
# Params
# ---------------------------
MAX_ROWS = None      # None = all
SLEEP_SEC = 1.5
results = []

# ---------------------------
# Main loop – Zero-shot prediction
# ---------------------------
for idx, row in ae_df.iloc[:MAX_ROWS].iterrows():
    ...
    # init defaults (avoid NameError in except)
    true_soc_term_primary = None
    true_soc_code_primary = None
    pred_soc_term = None
    pred_pt_term = None
    ...
    prompt = (
        ...
        "Your task is to assign ONE MedDRA Low Level Term (LLT) from the MedDRA dictionary (version 25.0) "
        "that best matches the following adverse event description.\n\n"
        f"Adverse event description:\n\"{ae_text}\"\n\n"
        "Respond with exactly one MedDRA LLT term, without any explanation, on a single line."
    )

    try:
        full_answer = call_llm_zero_shot(prompt, model=MODEL_NAME, retries=3, sleep_sec=1.0)
        raw_last_line = full_answer.split("\n")[-1].strip()
        pred_llt_term = clean_model_term(raw_last_line)

        # validity: does this term exactly match a MedDRA LLT?
        key = pred_llt_term.lower()
        pred_llt_code = map_pred_term_to_llt_code(pred_llt_term)
        is_valid_meddra_llt = pred_llt_code is not None
        ...
        # derive PT/SOC if mappings are available
        ...
        results.append({
            ...
            "is_valid_meddra_llt": bool(is_valid_meddra_llt),
        ...
        print(f"→ Valid MedDRA LLT? {is_valid_meddra_llt}")
...
y_pred_fuzzy = [
    (r.get("true_llt_term") or r.get("pred_llt_term") or "")
    if r.get("fuzzy_match")
    else (r.get("pred_llt_term") or "")
    for r in results
]
...
valid_rate = (sum(1 for r in results if r.get("is_valid_meddra_llt")) / len(results)) if results else 0.0

print(f"\nAccuracy (exact LLT term): {acc:.4f}")
print(f"F1 Score (macro):          {f1:.4f}")
print(f"Precision (macro):         {precision:.4f}")
print(f"Recall (macro):            {recall:.4f}")
print(f"Fuzzy Match Accuracy:      {fuzzy_accuracy:.4f}")
print(f"Valid MedDRA LLT rate:     {valid_rate:.4f}")
...
metrics_payload = {
    "meta": {
        "dataset": os.path.basename(AE_CSV_PATH).replace(".csv", ""),
        "model": MODEL_NAME,
        "baseline": "zero-shot",
        "max_rows": MAX_ROWS,
        "seed": RUN_SEED,
        "timestamp": time.strftime("%Y-%m-%dT%H:%M:%S"),
        "notes": "Zero-shot baseline: no candidate list; validity vs MedDRA dictionary recorded; retry+null-safe enabled."
    },
    "counts": {
        "n_samples": len(results),
        "n_pt_pairs": len(_both_present([(r.get('true_pt_code'), r.get('pred_pt_code')) for r in results])) if has_pt_in_llt else 0,
        "n_soc_optA": len(soc_pairs_ae) if soc_pairs_ae is not None else 0,
        "n_soc_optB": len(soc_pairs_primary) if soc_pairs_primary is not None else 0,
    ...
    "metrics": {
        ...
        "PT_code_acc": (float(pt_acc) if pt_acc is not None else None),
        "SOC_acc_option_a": (float(soc_acc_vs_ae) if soc_acc_vs_ae is not None else None),
        "SOC_acc_option_b": (float(soc_acc_vs_true_primary) if soc_acc_vs_true_primary is not None else None),
        "SOC_acc_any_of_vs_AE": (float(soc_acc_any) if soc_acc_any is not None else None),
        "Valid_MedDRA_LLT_rate": float(valid_rate),
    },
}
