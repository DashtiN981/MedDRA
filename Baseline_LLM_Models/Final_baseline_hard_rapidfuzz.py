"""
=== File Name: Final_baseline_hard_rapidfuzz.py     === Author: Naghme Dashti / August 2025
...
PT_CSV_PATH  = "/home/naghmedashti/MedDRA-LLM/data/PT2_SOC_25_0.csv"  # expects PT_Code, SOC_Code; PT_Term, SOC_Term, Ist_Primary_SOC optional
...
# ===========================
# Load PT/SOC mapping (respects Ist_Primary_SOC = Y/N)
# ===========================
pt_code_to_term = {}
soc_code_to_term = {}
...
try:
    ...
    keep_pt = [c for c in ["PT_Code", "SOC_Code", "PT_Term", "SOC_Term", "Ist_Primary_SOC"] if c in pt_df.columns]
    ...
    has_primary_col = "Ist_Primary_SOC" in pt_df.columns
    if has_primary_col:
    ...
    # Fallbacks:
    # - If PT has exactly one SOC → take that SOC as primary.
    # - If PT has multiple SOCs but no 'Y' → leave primary as None (undefined).
    ...
    num_with_y = 0
    if has_primary_col:
        # count PTs that have at least one Y row
...
for idx, row in ae_df.iloc[:MAX_ROWS].iterrows():
    ...
    try:
        ...
        # Flags to see if primary is undefined for true/pred PT
        ...
        results.append({
            ...
            "true_soc_code": true_soc_code_ae,               # from AE if present, else from mapping primary
            "pred_soc_code": pred_soc_code,                  # predicted primary from mapping
            # Also keep the mapping "true primary" explicitly for clarity
            ...
            "true_soc_term_primary": true_soc_term_primary,  # mapping primary SOC term
